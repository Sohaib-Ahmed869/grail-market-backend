...
export async function shape(
  ...
  return {
    ...
    // From settings, not constants: GM001-34 asks for the threshold to be a
    // setting so it can be relaxed once automatic approval proves reliable.
    // `floors` is already threaded in from tierFloors(); these two were the
    // last references to the constants that used to hold them.
    tier:
      ask >= floors.grailFloor
        ? "grail"
        : ask >= floors.highValueFloor
          ? "high-value"
          : "standard",
